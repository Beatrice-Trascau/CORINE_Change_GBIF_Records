...
# 3.4_intens_extens_cover_change_occ_model_offset_setup
# This script contains code which runs the simple models looking at the effect 
# of land cover change (intensification/extensification) and offset on the
# number of occurrences in a pixel
##----------------------------------------------------------------------------##
...
subset_data_intens_extens_before_after <- occ_intens_extens_before_after_for_model |>
  sample_frac(0.1)

# Save subset
save(subset_data_intens_extens_before_after, 
     file = here::here("data", "derived_data", 
                       "subset_data_intens_extens_before_after.rda"))
...
# Run model
model4.1 <- glmmTMB(ocurrences_after ~ cover_change * time_period * ocurrences_before + offset(ocurrences_before) + (1 | SSBID),
                    family = nbinom2,
                    data = subset_data_intens_extens_before_after)

# Save model output to file to save time next time
save(model4.1, file = here::here("data", "models", "model4.1.RData"))

## 2.2. N binomial glmmTMB, nbinom 2, Municipality on data subset --------------

# Run model
model4.2 <- glmmTMB(ocurrences_after ~ cover_change * time_period * ocurrences_before + offset(ocurrences_before) + (1 | municipality),
                    family = nbinom2,
                    data = subset_data_intens_extens_before_after)

# Save model output to file to save time next time
save(model4.2, file = here::here("data", "models", "model4.2.RData"))
